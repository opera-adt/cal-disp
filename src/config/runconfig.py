...
class OutputOptions(BaseModel, extra="forbid"):
    ...
    date_fmt: str = Field(
        "%Y%m%dT%H%M%S",
        description="Output Date Format for OPERA CAL-DISP",
    )
    ...
    chunk_size: tuple[int, int, int] = Field(
        (1, 64, 64),
        description="Ouput chunks (time, lat, lon).",
    )
    ...
    product_version: str = Field(
        PRODUCT_VERSION,
        description="OPERA CAL-DISP product version",
    )
    ...
    def get_output_filename(self, date: str | datetime, hour: str | int):
        ...
        return (
            f"OPERA_L4_CAL-DISP_{datetime_str}_{datetime_str}_{self.product_version}.nc"
        )
...
class CalibrationWorkflow(YamlModel, extra="forbid"):
    """DISP Calibration configuration models."""
    ...
    log_file: Optional[Path] = Field(
        ...
        description=(
            ...
            " Default logs to `cal_disp.log` within `work_directory`"
        ),
    )
